defmodule AlertsViewerWeb.CoreComponents do
  ...
  attr(:stream, :boolean,
    default: false,
    required: false,
    doc: "whether the table is made using livestreams"
  )
  ...
  def table(assigns) do
    # Make non-stream rows same shape as stream rows
    assigns =
      case assigns.stream do
        true -> assigns
        false -> assign(assigns, :rows, Enum.map(assigns.rows, fn row -> {nil, row} end))
      end
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="mt-11 w-[40rem] sm:w-full">
        ...
        >
          <tr :for={{dom_id, row} <- @rows} id={dom_id} class="group hover:bg-zinc-50">
            <td
